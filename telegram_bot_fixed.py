...
async def channel_info_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for m in members:
        ...
        uname = f"@{m['username']}" if m.get("username") else f"`{m['user_id']}`"
        lines.append(f"• {uname} — joined {joined}")
...
# NOTE: reconstructed — verify these db function names/signatures against
# your actual database.py, since the original tracking handler was not
# present in the file provided.
# ---------------------------------------------------------------------------
...
async def track_chat_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if not was_member and is_member:
            ...
            # user left/kicked
            db.remove_channel_member(channel_id=chat.id, user_id=user.id)
    except Exception as e:
